  ...
  EreignisbuchungIstDeleted = (deletedBuchung) => {
    this.setState({
      ereignisbuchungen: this.state.ereignisbuchungen.filter(
        (buchung) => buchung.id != deletedBuchung.id
      ),
      deletedIstTrue: true,
    });
  };
  ...
  render() {
    ...
    return (
      <div>
        {istBuchung ? (
          <div>
            <button onClick={this.getEreignisbuchungIST} class="filterBtn">
              Suche
            </button>
            <div class="buchungDeleted">
              {deletedIstTrue ? (
                <Stack sx={{ width: "100%" }} spacing={2}>
                  <Alert onClose={this.handleDeletedIstClose}>
            ...
            {ereignisbuchungen
              ? ereignisbuchungen.map((ereignisbuchung) => (
                  <EreignisbuchungListEntry
                    key={ereignisbuchung.getID()}
                    ereignisbuchung={ereignisbuchung}
                    istBuchung={true}
                    ereignisbuchungIstDeleted={this.EreignisbuchungIstDeleted}
                  />
                ))
              : null}
          </div>
          ...
          <div>
            <button onClick={this.getEreignisbuchungSOLL} class="filterBtn">
              Suche
            </button>
            <div class="buchungDeleted">
              {deletedSollTrue ? (
                <Stack sx={{ width: "100%" }} spacing={2}>
                  <Alert onClose={this.handleDeletedSollClose}>
                    Buchung erfolgreich gelöscht!
                  </Alert>
                </Stack>
              ) : null}
            ...
            {ereignisbuchungenSoll
              ? ereignisbuchungenSoll.map((ereignisbuchung) => (
                  <EreignisbuchungListEntry
                    key={ereignisbuchung.getID()}
                    ereignisbuchung={ereignisbuchung}
                    istBuchung={true}

...
//schau ob die Abwesenheit Ein Ende oder kein Ende haben soll
  ...
  addAbwesenheitsEnde = (ereignis) => { 
    let zeitpunkt =
      this.state.abwesenheitsart == "Gleitzeit"
        ...
        : this.state.abwesenheitsZeit2 + "T23:59";
    let newAbwesenheitEnde = new EreignisBO(
      zeitpunkt,
      this.state.abwesenheitsart
    );
    OneAPI.getAPI()
      .addAbwesenheitEnde(
        newAbwesenheitEnde,
      ...
      .then((ereignis) => this.props.onClose())
      .catch(
        (e) =>
          this.setState({
  ...
  addAbwesenheitsBeginn = () => {
    let zeitpunkt =
      this.state.abwesenheitsart == "Gleitzeit"
        ? this.state.abwesenheitsZeit1
        : this.state.abwesenheitsZeit1 + "T00:00";
    let newAbwesenheitBeginn = new EreignisBO(
      zeitpunkt,
      this.state.abwesenheitsart
    );
    OneAPI.getAPI()
      .addAbwesenheitBeginn(
        newAbwesenheitBeginn,
        this.props.user[0].id,
        this.state.abwesenheitsart
      )
      .then((ereignis) => {
  ...
  render() {
    ...
    const { abwesenheitsart, abwesenheitsZeit1, abwesenheitsZeit2 } =
      this.state;
    return show ? (
      <Dialog
        open={show}
          ...
          <DialogContentText>
            ...
            {abwesenheitsart == "Krankheitsausfall" ? (
              <FormControlLabel
                ...
                control={
                  <Checkbox onChange={this.handleAbwesenheitOhneEndeChecked} />
                }
                label="Abwesenheit ohne Ende erfassen"
              />
            ) : abwesenheitsart == "Krankheitsausfall ohne Ende" ? (
              <FormControlLabel
                sx={{ marginLeft: 1, marginTop: 1 }}

...
class ArbeitszeitkontoMapper(Mapper):
    ...
    def find_by_key(self, key):
        ...
        cursor = self._cnx.cursor()
        command = "SELECT id, timestamp, user, urlaubskonto, arbeitsleistung, gleitzeit FROM arbeitszeitkonto WHERE user={}".format(
            key
        )
        cursor.execute(command)
        tuples = cursor.fetchall()
    ...
    def update(self, arbeitszeitkonto: Arbeitszeitkonto) -> Arbeitszeitkonto:
        ...
        data = (
            arbeitszeitkonto.get_timestamp(),
            arbeitszeitkonto.get_user(),
            arbeitszeitkonto.get_urlaubskonto(),
            arbeitszeitkonto.get_arbeitsleistung(),
            arbeitszeitkonto.get_gleitzeit(),
            arbeitszeitkonto.get_user(),
        )
    ...
    def delete(self, arbeitszeitkonto):
        ...
        command = "DELETE FROM arbeitszeitkonto WHERE id={}".format(
            arbeitszeitkonto.get_id()
        )
        cursor.execute(command)
